...
const AgenticLoop = () => {
  const steps = [
    { icon: Target, label: "Set retention goal", number: 1 },
    { icon: Brain, label: "Agent reads user state", number: 2 },
    { icon: Zap, label: "Chooses best retention action", number: 3 },
    { icon: Play, label: "Executes instantly", number: 4 },
    { icon: BarChart3, label: "Measures retention outcome", number: 5 },
    { icon: Database, label: "Updates Retention DNA", number: 6 },
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto text-center mb-20">
          ...
          <p className="text-xl text-muted-foreground leading-relaxed">
            How One Retention Goal Drives Millions of Per-User Decisions
          </p>
        </div>
            ...
            {steps.map((step, index) => (
              <div key={index} className="relative">
                ...
                {index < steps.length - 1 && (
                  <div className="hidden md:block absolute -right-1.5 top-1/2 transform -translate-y-1/2 z-10 text-primary/40">→</div>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="max-w-4xl mx-auto text-center mb-16">
          <div className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-primary/10 via-primary/5 to-primary/10 border border-primary/30 rounded-full">
            ...
            <span className="text-base font-semibold text-primary">Continuous Retention Loop</span>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto">
          <Card className="p-10 md:p-12 bg-gradient-to-br from-card to-muted/30 border-border">
            <p className="text-lg md:text-xl text-foreground mb-6 leading-relaxed">
              With agents running retention experiments autonomously, your team focuses on <span className="font-semibold">setting goals, defining guardrails,</span> and reviewing the behavioral insights AI surfaces about your subscriber base.
            </p>
            <div className="h-px bg-border mb-6" />
            <p className="text-lg text-muted-foreground italic leading-relaxed">
              Agents continuously surface emerging patterns — shifts in engagement, renewal timing windows, churn risk clusters — enriching your retention strategy each cycle.
            </p>
          </Card>
        </div>
      </div>
